...
export default function FeedCreationForm({
  feedDataToUpdate = {},
  actionType = "add",
  closeEditModal,
}) {
  const { createFeed, updateFeed } = useFeedContext();

  const [feedData, setFeedData] = useState({
    body: feedDataToUpdate?.body || "",
    visibility: feedDataToUpdate?.visibility || "public",
    image: feedDataToUpdate?.image || null,
  });

  const fileInputRef = useRef(null);
  const [preview, setPreview] = useState(feedDataToUpdate?.image || null);

  const handleFeedDataForm = (e) => {
    setFeedData({
      ...feedData,
  ...
  const handleUpdateFormVisibility = (e) => {
    setFeedData({
      ...feedData,
  ...
  const handleFeedCreationFormSubmit = async (e) => {
    e.preventDefault();

    const formData = new FormData();

    formData.append("body", feedData.body);
    formData.append("visibility", feedData.visibility.toLowerCase());

    if (feedData.image instanceof File) {
    ...
    if (actionType === "add") {
      ...
      await updateFeed(feedDataToUpdate.id, formData);
      closeEditModal();
    }
    setFeedData({
      body: "",
    ...
    <div className="_feed_inner_text_area  _b_radious6 _padd_b24 _padd_t24 _padd_r24 _padd_l24 _mar_b16">
      <div className="_feed_inner_text_area_box">
        ...
        <div className="form-floating _feed_inner_text_area_box_form ">
          <textarea
            className={`form-control _textarea ${feedData.body ? "_feed_textarea_active" : ""}`}
            placeholder="Leave a comment here"
            id="floatingTextarea"
            name="body"
            value={feedData.body}
            onChange={handleFeedDataForm}
          ></textarea>
          {preview && (
            <img src={preview} alt="Preview" className="preview_image" />
          )}
          <label
            className={`_feed_textarea_label ${feedData.body ? "_feed_textarea_level_active" : ""}`}
          ...
          <div className="col-sm-10 d-flex gap-2">
            <div className="form-check">
              <input
                ...
                type="radio"
                name={`visibility-${actionType}`}
                id={`public-${actionType}`}
                value="public"
                checked={feedData.visibility.toLowerCase() === "public"}
                onChange={handleUpdateFormVisibility}
              />
              <label
                className="form-check-label"
                htmlFor={`public-${actionType}`}
              >
                Public
              </label>
            </div>
            <div className="form-check">
              <input
                ...
                type="radio"
                name={`visibility-${actionType}`}
                id={`private-${actionType}`}
                value="private"
                checked={feedData.visibility.toLowerCase() === "private"}
                onChange={handleUpdateFormVisibility}
              />
              <label
                className="form-check-label"
                htmlFor={`private-${actionType}`}
              >
                Private
              </label>
            </div>
      ...
      <div className="_feed_inner_text_area_bottom">
        <div className="_feed_inner_text_area_item">
          <div className="_feed_inner_text_area_bottom_photo _feed_common">
            ...
            >
              ...
              {actionType === "update" ? "Change" : ""} Photo
            </button>
        ...
        <div className="_feed_inner_text_area_btn">
          <button
            ...
            onClick={handleFeedCreationFormSubmit}
          >
            <svg
              className="_mar_img"
              xmlns="http://www.w3.org/2000/svg"
            ...
            <span> {actionType === "update" ? "Update" : ""} Post</span>
          </button>
        </div>
      </div>
